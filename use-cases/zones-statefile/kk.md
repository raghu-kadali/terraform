...
# store statefile in backend (already creted before)
...
# terraform {
 ...
 #-------------------------9th-questio---------------------------------------------------
  9. I need to configure the two VMs in different zones with a single codebase, and I need separate state
files? ---workspace
provider "google" {
  project = "terraform-project-470306"
  region  = "us-central1"
...
# runn time use
# terraform workspace select vm1
# terraform apply -var="zone=us-central1-a" -var="vm_name=vm-1"
...
# extra ideas only -bombard magics haha
#4th question incase single hi raghu print use these 

  # Write "Hi Raghu" into a separate file
  metadata_startup_script = <<-EOT
    #!/bin/bash
    echo "Hi Raghu" > /home/debian/greeting.txt
  EOT
}
